...
def default_output_path(fmt: PackFormat) -> Path:
    if fmt is PackFormat.MARKDOWN:
        return Path("code-skeleton-pack.md")
    if fmt is PackFormat.PLAIN:
        return Path("code-skeleton-pack.txt")
    if fmt is PackFormat.JSON:
        return Path("code-skeleton-pack.json")
    if fmt is PackFormat.JSONL:
        return Path("code-skeleton-pack.jsonl")
    return Path("code-skeleton-pack.xml")
