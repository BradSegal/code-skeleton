"""Command-line interface for code-skeleton.

This module provides the CLI for generating and validating code skeletons.

Usage
-----
    code-skeleton generate ./src --output .skeleton
...
from code_skeleton.pack.limit import OutputLimit, parse_output_limit
...
from code_skeleton.pack.tree import render_token_tree

app = typer.Typer(
    name="code-skeleton",
    help="Generate token-efficient codebase structure maps for AI agents.",
...
def version_callback(value: bool) -> None:
    ...
    if value:
        ...
        typer.echo(f"code-skeleton {__version__}")
        raise typer.Exit()
...
def main(
    ...
    """Code skeleton generator for AI agents."""
...
def _resolve_sources(cli_sources: list[Path], config: SkeletonConfig | None) -> list[Path]:
    if cli_sources:
        return cli_sources
    if config is None:
        raise typer.BadParameter("No sources provided and no .code-skeleton.yaml found")
...
def generate(
    sources: Annotated[
        list[Path],
        typer.Argument(
            help="Source directory(ies) to analyze. If omitted, uses .code-skeleton.yaml sources.",
            exists=True,
            file_okay=False,
            dir_okay=True,
            resolve_path=True,
    ...
    config: Annotated[
        Path | None,
        typer.Option("--config", help="Path to .code-skeleton.yaml (overrides auto-discovery).", exists=True),
    ] = None,
    output: Annotated[
        Path | None,
        typer.Option(
    ...
    """Generate code skeleton for a source directory.

    Examples
    --------
        code-skeleton generate ./src
        code-skeleton generate ./src --output .skeleton --level signatures
        code-skeleton generate ./src -f yaml -f json -f markdown
    """
    try:
...
def estimate(
    ...
    Examples
    --------
        code-skeleton estimate ./src
        code-skeleton estimate ./src --level signatures
    """
    try:
        cfg = _load_config_from(config)
        resolved_sources = _resolve_sources(sources, cfg)
...
def validate(
    ...
    --------
        code-skeleton validate .skeleton --source ./src
        code-skeleton validate .skeleton -s ./src -s ./tests --fix
    """
    try:
        from code_skeleton.validation import validate_skeleton_dir
...
def pack(
    ...
    Examples
    --------
        code-skeleton pack . --format markdown --output pack.md
        code-skeleton pack . --include 'src/**' --ignore '**/__pycache__/**'
        code-skeleton pack . --entry src/code_skeleton/cli.py --deps --compress
    """
    try:
        from code_skeleton.config import PackConfig
        from code_skeleton.pack.formats import infer_pack_format_from_output_path
        from code_skeleton.pack.runner import pack as run_pack
