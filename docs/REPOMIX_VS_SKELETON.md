# Repomix vs code-skeleton (local comparison)
...
- code-skeleton: `pack` full + `--compress`
...
# code-skeleton (full)
code-skeleton pack . --format markdown --output code-skeleton.full.md --token-encoding o200k_base

# code-skeleton (compress)
code-skeleton pack . --format markdown --output code-skeleton.compress.md --token-encoding o200k_base --compress
```
...
| `code-skeleton.full.md` | 227,186 | 50,578 |
| `code-skeleton.compress.md` | 75,771 | 18,640 |

### Notes
- The “full” outputs are similar in size and tokens on this repository.
- `code-skeleton pack --compress` is substantially smaller here because it emits a stricter Python-only structural summary (signatures + constants + imports) rather than a broader “essential code” extraction. This trades context for token savings.
- For slicing specific subsystems, `code-skeleton pack --entry ... --deps` can reduce output further by selecting only a dependency closure.

---

## Feature parity and ergonomics

### What `code-skeleton pack` matches (or extends)
...
- **Compression semantics**: `code-skeleton --compress` is Python-structural (very token-efficient) and intentionally trades off full bodies; repomix compression aims to keep more “essential code” context across languages.
- **Dependency knowledge**: repomix is primarily a bundler; `code-skeleton` adds explicit dependency slicing for Python.
- **Hybrid mode**: repomix does not have a first-class “summary everywhere, fill specific files” representation policy; `code-skeleton` uses this to reduce tokens while keeping navigability.
...
Yes:
- **Importers only**: `code-skeleton pack . --target path/to/mod.py --reverse-deps --output importers.md`
- **Importers plus what they import**: `code-skeleton pack . --target path/to/mod.py --reverse-deps --deps --output slice.md`
- **Reference-based usage**: `code-skeleton pack . --target path/to/mod.py --uses --slice-backend pyright --output uses.md`
